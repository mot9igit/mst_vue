      ...
      <div>
        ...
        <div class="dart-form-group mb-4">
          <div class="upload-banner">
            ...
            <FileUpload mode="basic" name="icon[]" :url="'/rest/file_upload.php?banner=icon'" accept="image/*" :maxFileSize="2000000" @upload="onUpload" :auto="true" chooseLabel="Загрузить" />
          </div>
        ...
        <div class="dart-form-group mb-4">
            <div class="rules-container">
                ...
                <FileUpload mode="basic" name="icon[]" :url="'/rest/file_upload.php?banner=file'" accept="application/pdf" :maxFileSize="20000000" @upload="onUpload" :auto="true" chooseLabel="Загрузить" />
            </div>
        </div>
                  ...
                  <img :src="'https://mst.tools' + item.image" alt="">
                    ...
                    <div class="PickList__values">
                        ...
                        <InputNumber
                            ...
                            @update:modelValue="setPrices(index, 'discount_money', this.selected[index].discount_money)"
                            incrementButtonIcon="pi pi-plus" decrementButtonIcon="pi pi-minus"
                        />
                        <InputNumber
                            ...
                            mode="currency" currency="RUB"
                            min="0"
                            @update:modelValue="setPrices(index, 'final_price', this.selected[index].final_price)"
                            incrementButtonIcon="pi pi-plus" decrementButtonIcon="pi pi-minus"
                        />
                    </div>
                  </div>
    ...
    setPrices (index, name, value) {
      switch (name) {
        case 'discount_proccent':
          ...
          break
        case 'discount_money':
          this.selected[index].discount_proccent = value / (Number(this.selected[index].price) / 100)
          this.selected[index].final_price = Number(this.selected[index].price) - this.selected[index].discount_money
          break
        case 'final_price':
          this.selected[index].discount_money = Number(this.selected[index].price) - value
          this.selected[index].discount_proccent = this.selected[index].discount_money / (Number(this.selected[index].price) / 100)
          break
      }
    },
    select (id) {
      const product = this.products.find(r => r.id === id)
      product.discount_proccent = 0
      product.discount_money = 0
      product.final_price = Number(product.price)
      console.log(product)
