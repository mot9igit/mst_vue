        ...
        <table class="kenost-actions-modal__table">
            ...
            <tbody>
              <tr v-for="item in this.actions_item.actions" v-bind:key="item.id" @click="this.active = !this.active" :class="{'active-el' : this.active, 'no-active-el' : !this.active}">
                ...
                <td class="kenost-actions-modal__center">Скидка <span v-if="item.old_price != 0">{{((item.new_price) / (item.old_price / 100)).toFixed(2)}}</span><span v-else>100</span>%, оплата доставки {{item.payer === '1' ? 'поставщиком' : 'покупателем'}}
